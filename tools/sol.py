...
def main():
    ...
    parser.add_argument(
        "target", action="store", type=str, choices=[
            "cf", "codeforces", "lc", "leetcode", "rm", "readme"
        ]
    )
    parser.add_argument(
        "action", action="store", type=str, choices=["p", "pull", "d", "delete", "con", "contest", "g", "generate"]
    )
    ...
    parser.add_argument('val', default='', type=str, nargs='+', help='')
    ...
    if target == "codeforces" and (action == "pull" or action == "delete" or action == "contest"):
    ...
    if action == "pull":
        ...
        t.contest(args.val[0], args.lang, os.path.join(sol_dir, "..", "contests"))
