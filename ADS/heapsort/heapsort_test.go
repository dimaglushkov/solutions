...
const (
	iterations           = 5
	minUnsortedDataSize  = 1000
	maxUnsortedDataSize  = 100000
	maxUnsortedDataValue = 1000000
)

var (
	benchmarkSizes = []int{100, 1000, 10_000, 100_000, 500_000}
	seed           int64
	rng            *rand.Rand
)
...
func BenchmarkHeapSortRandomData(b *testing.B) {
	for _, s := range benchmarkSizes {
		d := generateUnsortedData(s, s, maxUnsortedDataValue)
		HeapSort(d)
	}
}

func BenchmarkQuickSortRandomData(b *testing.B) {
	for _, s := range benchmarkSizes {
		d := generateUnsortedData(s, s, maxUnsortedDataValue)
		sort.Ints(d)
	}
